...
actions.updatePrompt = async (payload) => {
  ...
  if (api.assert(isSignedWithActiveKey === true, 'you must use a custom_json signed with your active key')
    ...
    if (prompt.user !== api.sender) {
      ...
      const updatedPrompt = await api.db.update('prompts', { _id: promptId }, {
        title, description, category, promptFile, budgetRange, skillsRequired,
      });
...
actions.getApplication = async (payload) => {
  ...
  if (api.assert(promptId && typeof promptId === 'string'
        && applicationId && typeof applicationId === 'string', 'invalid params')) {
